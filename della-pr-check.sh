...
then
    ...
    # Turn of ignore installed for conda environments since we are going to install mpi4py\mpich
    # via conda before pip installing brainiak and dependencies.
...
# On Della, I can't get openmpi or intel-mpi to work with tests. OpenMPI fails
# because of a sub-process fork() and intel-mpi crashes because of some floating
# point exception. Lets install mpi4py from conda (with mpich) for now, less than
# ideal.
...
python3 -m pip install --no-cache-dir $ignore_installed -U -e .[matnormal] || \
...
salloc -t 00:30:00 -N 1 -n 4 ./run-tests.sh $sdist_mode || \
