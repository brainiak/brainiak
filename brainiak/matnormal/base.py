...
class MatnormModelBase(BaseEstimator):
    ...
    def _optimize_impl(self, optfun, loss, optvars, feed_dict, max_iter, step,
        ...
        grad = tf.concat(0, [tf.reshape(g[0], [-1]) for g in
                         self.optimizer.compute_gradients(loss, optvars)])
    ...
    def _mnorm_logp_internal(self, colsize, rowsize, logdet_row, logdet_col,
                             solve_row, solve_col):
    ...
    def matnorm_logp(self, x, row_cov, col_cov):
        ...
        return self._mnorm_logp_internal(colsize, rowsize, logdet_row,
    ...
    def matnorm_logp_marginal_row(self, x, row_cov, col_cov, marg, marg_cov):
        ...
        solve_row, logdet_row = self.solve_det_marginal(x, row_cov, marg,
                                                        marg_cov)
    ...
    def matnorm_logp_marginal_col(self, x, row_cov, col_cov, marg, marg_cov):
        ...
        solve_col, logdet_col = self.solve_det_marginal(tf.transpose(x),
                                                        col_cov, marg, marg_cov)
    ...
    def matnorm_logp_conditional_row(self, x, row_cov, col_cov, cond, cond_cov):
        ...
        solve_row, logdet_row = self.solve_det_conditional(x, row_cov, cond,
                                                           cond_cov)
    ...
    def matnorm_logp_conditional_col(self, x, row_cov, col_cov, cond, cond_cov):
        ...
        solve_col, logdet_col = self.solve_det_conditional(tf.transpose(x),
                                                           col_cov, cond,
                                                           cond_cov)
