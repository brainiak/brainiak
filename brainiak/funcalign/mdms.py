...
def _init_w_transforms(voxels, features, random_states, datasets):
    """Initialize the mappings (W_s) for the MDMS with random orthogonal matrices.
    ...
    w : dict of array, w[s] has shape=[voxels[s], features] where s is the name
        of the subject. 
...
def _sanity_check(X, datasets, comm):
    """Check if the input data and datasets information have valid shape/configuration.
    ...
    ----------
        ...
        X : dict of list of 2D arrays or dict of dict of 2D arrays
            1) When it is a dict of list of 2D arrays:
                X[d] is a list of data of dataset d, where d is the name of the dataset.
            ...
            2) When it is a dict of dict of 2D arrays:
                X[d][s] has shape=[voxels_s, samples_d], which is the fMRI data of
                subject s in dataset d, where s is the name of the subject and
    ...
    samples_ : dict of int, samples_[d] is number of samples where d is the name
        of the dataset.
        A dict with the number of samples for each dataset.


    """
    # Check the number of subjects and all ranks have all datasets in the Dataset object
    ds_list = datasets.get_datasets_list()
    for (ds, ns) in datasets.num_subj_dataset.items():        
        if ns < 1:
            raise ValueError("Dataset {} should have positive num_subj_dataset".format(ds))                    
        if ds not in X:
            raise ValueError("Dataset {} not in all ranks".format(ds))
        if X[ds] is not None and len(X[ds]) < ns:
            raise ValueError("Dataset {} does not have enough subjects: Need equal to or more "
                             "than {0:d} subjects but got {0:d} to train the model.".format(ds, ns, len(X[ds])))
            ...
            for subj in range(datasets.num_subj):
                if datasets.dok_matrix[subj,ds_idx] != 0:
                    if datasets.built_from_data:
                        ...
                        if not idx in X[ds]:
                            raise Exception('Subject {} in dataset {} is missing.'.format(idx, ds))
                    else:
                        idx = datasets.dok_matrix[subj,ds_idx] - 1
                        if len(X[ds]) <= idx:
                            raise ValueError("Dataset {} does not have enough subjects: Need more "
                                 "than {0:d} subjects but got {0:d} to train the model.".format(ds, idx, len(X[ds])))
        ...
        for subj in range(datasets.num_subj):
            if datasets.dok_matrix[subj,ds_idx] != 0:
                if data_exist[ds][subj] == 0:
                    raise ValueError("Data of subject {} in dataset {} is missing.".format(datasets.dok_matrix[subj,ds_idx]-1, ds))
                elif data_exist[ds][subj] > 1:
                    raise ValueError("Data of subject {} in dataset {} appears more than once.".format(datasets.dok_matrix[subj,ds_idx]-1, ds))
            else:
                shape0[ds][subj] = 0
                shape1[ds][subj] = 0

    # Check if each subject has same number of voxels across different datasets
    voxels_ = {}
    for subj in range(datasets.num_subj):
        ...
        if any([v != voxels_[subj_name] for v in all_vxs_tmp]):
            raise ValueError("Subject {} has different number of voxels across"
                             "datasets.".format(subj_name))
...
class MDMS(BaseEstimator, TransformerMixin):
    ...
    Given multi-dataset multi-subject data, factorize it as a shared response S among all
    subjects per dataset and an orthogonal transform W across all datasets per subject:

    .. math:: X_{ds} \\approx W_s S_d, \\forall s=1 \\dots N, \\forall d=1 \\dots M
    ...
    sigma_s_ : dict of array, sigma_s_[d] has shape=[features, features]
        The covariance of the shared response Normal distribution for each dataset.
    ...
    rho2_ : dict of dict of float, rho2_[d][s] is a float, where d is the name 
        of the dataset and s is the name of the subject.
        The estimated noise variance :math:`\\rho_{di}^2` for each subject in each dataset.
    ...
    ----
       ...
       The number of voxels may be different between subjects within a dataset
       and number of samples may be different between datasets. However, the
       number of samples must be the same across subjects within a dataset and 
       number of voxels must be the same across datasets for the same subject.

       The probabilistic multi-dataset multi-subject model is approximated using the
       Expectation Maximization (EM) algorithm proposed in [Zhang2018]_. The
       ...
       all datasets, and K - the number of features (typically, :math:`V \\gg T \\gg K`).
    """
    ...
    def fit(self, X, datasets=None, y=None):
        """Compute the probabilistic multi-dataset multi-subject (MDMS) SRM analysis
        ...
        if format_X != dict and format_X != list:
            raise Exception('X should be a dict of dict of arrays or dict of list of arrays.')
        if format_X == list and (datasets is None or datasets.built_from_data is None or datasets.built_from_data):
            raise Exception("Argument 'datasets' must be defined and built from json "
        ...
        # Run MDMS
        self.sigma_s_, self.w_, self.mu_, self.rho2_, self.s_ = self._mdms(X, datasets)
    ...
    def transform(self, X, subjects, centered=True, y=None):
        ...
        X : list of 2D arrays, element i has shape=[voxels_i, samples_i]
            Each element in the list contains the new fMRI data of one subject
        ...
        # Check if the subject exist in the fitted model and has the right number of voxels
        for idx in range(len(X)):
            if not subjects[idx] in self.w_:
                raise NotFittedError("The model has not been fitted to subject {}.".format(subjects[idx]))
            if X[idx] is not None and self.w_[subjects[idx]].shape[0] != X[idx].shape[0]:
                raise ValueError("{}-th element of data has inconsistent number of"
                    "voxels with fitted model. Model has {} voxels while data has {}"
                    ".".format(idx, self.w_[subjects[idx]].shape[0], X[idx].shape[0]))
        ...
        for idx in range(len(X)):
            if X[idx] is not None:
                if centered:
                    ...
                    s[idx] = self.w_[subjects[idx]].T.dot(X[idx]-self.mu_[subjects[idx]][:, None])

        return s


    def _init_structures(self, data, datasets):
        ...
        x : dict of dict of array, x[d][s] has shape=[voxels[s], samples[d]]
            where d is the name of the dataset and s is the name of the subject
        ...
        rho2 : dict of dict of float, rho2_[d][s] is a float, where d is the name 
            of the dataset and s is the name of the subject.
            The estimated noise variance :math:`\\rho_{di}^2` for each subject in each dataset.
        ...
        for ds_idx, ds in datasets.idx_to_dataset.items():
            ...
            for subj in range(datasets.num_subj):
                if datasets.dok_matrix[subj,ds_idx] != 0:
                    if datasets.built_from_data:
                        x[ds][datasets.idx_to_subject[subj]] = data[ds][datasets.idx_to_subject[subj]]
                    else:
                        x[ds][datasets.idx_to_subject[subj]] = data[ds][datasets.dok_matrix[subj,ds_idx]-1]
        del data
        ...
        for subj in datasets.subject_to_idx.keys():
            for ds in mu_tmp[subj].keys():
                mu_tmp[subj][ds] = self.comm.allreduce(mu_tmp[subj][ds], op=MPI.SUM)
                weights[subj][ds] = self.comm.allreduce(weights[subj][ds], op=MPI.SUM)
        # compute final mean
        for subj in datasets.subject_to_idx.keys():
    ...
    def transform_subject(self, X, dataset):
        ...
        if not dataset in self.s_:
            raise NotFittedError("Dataset {} is not in the model yet.".format(dataset))
    ...
    def _mdms(self, data, datasets):
        """Expectation-Maximization algorithm for fitting the probabilistic MDMS.
        ...
        sigma_s : dict of array, sigma_s[d] has shape=[features, features] where
            d is the name of dataset.
            The covariance :math:`\\Sigma_s` of the shared response Normal
            distribution for each dataset.

        w : dict of array, w[s] has shape=[voxels_[s], features] where s is the name
            of the subject.
            The orthogonal transforms (mappings) :math:`W_s` for each subject.
        ...
        rho2 : dict of dict of float, rho2[d][s] is a float, where d is the name 
            of the dataset and s is the name of the subject.
            The estimated noise variance :math:`\\rho2{di}^2` for each subject 
            in each dataset.

        s : dict of array, s[d] has shape=[features, samples_[d]] where d is the
            name of the dataset.
            The shared response for each dataset.
        """
        # get information from datasets structures
        ds_list, subj_list = datasets.get_datasets_list(), datasets.get_subjects_list()
        ...
        random_states = {
        subj_list[i] : np.random.RandomState(self.random_state_.randint(2 ** 32)) 
        ...
        if datasets.num_dataset <= size:
            ...
            if rank != size - 1:
                ds_rank.update(set(ds_list[ds_rank_len*rank:ds_rank_len*(rank+1)]))
            else:
        ...
        for subj in subj_list:
            for ds in ds_subj_list[subj]:
                trace_xtx[ds][subj] = self.comm.allreduce(trace_xtx[ds][subj], op=MPI.SUM)

        shared_response, sigma_s, rho0 = {}, {}, {}
        for ds in ds_list:
            shared_response[ds] = np.zeros((self.features, self.samples_[ds]))          
            if ds in ds_rank:
        ...
        for iteration in range(self.n_iter):
            ...
            # E-step and some M-step: update shared_response and sigma_s of each dataset
            loglike = 0.

            # for multi-thread computation
            chol_sigma_s, chol_sigma_s_rhos, inv_sigma_s_rhos = {}, {}, {}
            wt_invpsi_x,  trace_xt_invsigma2_x, trace_sigma_s = {}, {}, {}
            for ds in ds_list:
                chol_sigma_s[ds] = np.zeros((self.features, self.features))
            ...
            for ds in ds_rank:
                ...
                inv_sigma_s = scipy.linalg.cho_solve(
                    (chol_sigma_s[ds], lower_sigma_s), np.identity(self.features),
                    check_finite=False)

                # Invert (Sigma_s[ds] + rho_0 * I) using Cholesky factorization
                sigma_s_rhos = inv_sigma_s + np.identity(self.features) * rho0[ds]
            ...
            for ds in ds_list:
                chol_sigma_s[ds] = self.comm.allreduce(chol_sigma_s[ds], op=MPI.SUM)
                chol_sigma_s_rhos[ds] = self.comm.allreduce(chol_sigma_s_rhos[ds], op=MPI.SUM)
                inv_sigma_s_rhos[ds] = self.comm.allreduce(inv_sigma_s_rhos[ds], op=MPI.SUM)
            ...
            for ds in ds_list:
                for subj in subj_ds_list[ds]:
                    if x[ds][subj] is not None:
                        wt_invpsi_x[ds] += (w[subj].T.dot(x[ds][subj])) / rho2[ds][subj]
                        trace_xt_invsigma2_x[ds] += trace_xtx[ds][subj] / rho2[ds][subj]

            # collect data from all ranks
            for ds in ds_list:
                wt_invpsi_x[ds] = self.comm.allreduce(wt_invpsi_x[ds], op=MPI.SUM)
                trace_xt_invsigma2_x[ds] = self.comm.allreduce(trace_xt_invsigma2_x[ds],
                                                        op=MPI.SUM)

            # compute shared response and Sigma_s of ds in this rank
            for ds in ds_list:
                if ds in ds_rank:
                    log_det_psi = np.sum([np.log(rho2[ds][subj])*self.voxels_[subj] for subj in rho2[ds]])

                    # Update the shared response
                    shared_response[ds] = sigma_s[ds].dot(
                        np.identity(self.features) - rho0[ds] * inv_sigma_s_rhos[ds]).dot(
                        wt_invpsi_x[ds])

                    # Update Sigma_s and compute its trace
                    sigma_s[ds] = (inv_sigma_s_rhos[ds]
                               + shared_response[ds].dot(shared_response[ds].T) / self.samples_[ds])
                    trace_sigma_s[ds] = self.samples_[ds] * np.trace(sigma_s[ds])
                    
                    # calculate log likelihood to check convergence
                    loglike += self._likelihood(
                        chol_sigma_s_rhos[ds], log_det_psi, chol_sigma_s[ds],
                        trace_xt_invsigma2_x[ds], inv_sigma_s_rhos[ds], wt_invpsi_x[ds],
                        self.samples_[ds])
            ...
            for ds in ds_list:
                shared_response[ds] = self.comm.allreduce(shared_response[ds], op=MPI.SUM)
                trace_sigma_s[ds] = self.comm.allreduce(trace_sigma_s[ds], op=MPI.SUM)
            ...
            for subj in subj_list:
                ...
                for ds in ds_subj_list[subj]:
                    if x[ds][subj] is not None:
                        trace_xtws_tmp[ds] = np.trace(x[ds][subj].T.dot(w[subj]).dot(shared_response[ds]))
                    else:
                        trace_xtws_tmp[ds] = 0.0
                # collect trace_xtws_tmp in all ranks
                for ds in ds_subj_list[subj]:
                    trace_xtws_tmp[ds] = self.comm.allreduce(trace_xtws_tmp[ds], op=MPI.SUM)
                # compute rho2
                if rank == 0:
                    for ds in ds_subj_list[subj]:
                        ...
                        rho2[ds][subj] /= self.samples_[ds] * self.voxels_[subj]                       
    ...
    def save(self, file):
        ...
        print ('MDMS object saved to {}.'.format(file))
        return 
    ...
    def restore(self, file):
        ...
        self.random_state_ = np.random.RandomState(self.rand_seed)
        self.logger = logger
        print ('MDMS object restored from {}.'.format(file))
        return


class DetMDMS(BaseEstimator, TransformerMixin):
    """Deterministic multi-dataset multi-subject (MDMS) SRM analysis (DetMDMS)
    ...
    ----
        ...
        The probabilistic multi-dataset multi-subject model is approximated using the
        Block Coordinate Descent (BCD) algorithm proposed in [Zhang2018]_. 
        ...
        sum of number of voxels from all subjects, T - the sum of number of 
        samples from all datasets, K - the number of features (typically, 
    ...
    def transform(self, X, subjects, centered=True, y=None):
        ...
        if not centered and self.mu_ is None:
            raise Exception('Mean values are not computed during model fitting. '
                'Please center the data to be transformed beforehand.')
    ...
    def _objective_function(self, data, subj_ds_list, w, s, num_sample):
        ...
        data : dict of dict of array, x[d][s] has shape=[voxels[s], samples[d]]
            where d is the name of the dataset and s is the name of the subject
            Demeaned data for each subject.
        ...
        num_sample : int, total number of samples across all datasets and datasets
        ...
        for ds in subj_ds_list.keys():
            for subj in subj_ds_list[ds]:
                if data[ds][subj] is not None:
                    objective += \
                        np.linalg.norm(data[ds][subj] - w[subj].dot(s[ds]), 'fro') ** 2
    ...
    def _mdms(self, data, datasets, demean):
        """Block Coordinate Descent algorithm for fitting the deterministic MDMS.
        ...
        data : dict of list of 2D arrays or dict of dict of 2D arrays
            1) When it is a dict of list of 2D arrays:
                data[d] is a list of data of dataset d, where d is the name of the dataset.
                Element i in the list has shape=[voxels_i, samples_[d]] 
                which is the fMRI data of the i'th subject in dataset d.
            2) When it is a dict of dict of 2D arrays:
                data[d][s] has shape=[voxels_[s], samples_[d]], which is the fMRI data of
                subject s in dataset d, where s is the name of the subject and
                d is the name of the dataset.
        ...
        subj_ds_list = datasets.subjects_in_dataset_all()
        ds_subj_list = datasets.datasets_with_subject_all()
        num_sample = np.sum([datasets.num_subj_dataset[ds]*self.samples_[ds] for ds in ds_list])
        ...
        if self.logger.isEnabledFor(logging.INFO):
            ...
            objective = self._objective_function(x, subj_ds_list, w, shared_response, num_sample)
    ...
    def save(self, file):
        ...
        with open(file, 'wb') as f:
            pkl.dump(data, f, pkl.HIGHEST_PROTOCOL)
        print ('DetMDMS object saved to {}.'.format(file))
        return 
    ...
    def restore(self, file):
        ...
        self.random_state_ = np.random.RandomState(self.rand_seed)
        self.logger = logger
        print ('DetMDMS object restored from {}.'.format(file))
        return
...
class Dataset(object):
    ...
    file : (optional) string, default: None
        ...
        Each JSON file should contain a dict or a list of dict where each dict
        has information of one dataset. Each dict must have 'dataset', 
        'num_of_subj', and 'subjects' where 'dataset' is the name of the dataset, 
        'num_of_subj' is the number of subjects in the dataset, and 'subjects' 
        is a list of strings with names of subjects in the dataset in the same 
        order as in the dataset. All datasets in all JSON files will be added 
        to the organizer. 
    ...
    dataset_to_idx : dict of int, dataset_to_idx[d] is the column index of dataset d
        in self.matrix, where d is the name of the dataset.
        Dataset name to column index of matrix, 0-indexed

    idx_to_dataset : dict of string, idx_to_dataset[i] is name of the dataset mapped 
        to the i'th column in self.matrix.
        Column index of metrix to dataset name, 0-indexed

    subject_to_idx : dict of int, subject_to_idx[s] is the row index of subject s
        in self.matrix, where s is the name of the subject.
        Subject name to row index of matrix, 0-indexed

    idx_to_subject : dict of string, idx_to_subject[i] is name of the subject mapped 
        to the i'th row in self.matrix.
        Row index to subject name, 0-indexed

    connected : list of list of string, each element is a list of name of connected
        datasets (datasets can be connected through shared subjects). 
    ...
    adj_matrix : 2D csc sparse matrix of shape [num_dataset, num_dataset], 
        Weighted adjacency matrix of all datasets, where each node is a dataset and 
        weights on edges are number of shared subjects between the two datasets. 
        Mapping between dataset name and dataset index is in self.dataset_to_idx.
    
    num_subj_dataset : dict of int, num_subj_dataset[d] is an int where d is the name
        of a dataset.
        Number of subjects of each dataset

    subj_in_dataset : dict of list of string, subj_in_dataset[d] is a list of name 
        of subjects in dataset d in the same order as in d, where d is the name
        of a dataset. If any subject is removed from the organizer, the name will 
        be replaced with None as a placeholder.
    ...
    matrix : 2D coo sparse matrix of shape [num_subj, num_dataset], 
        ...
        If built from multi-dataset multi-subject data, self.matrix[i,j] = 1 if 
        subject self.idx_to_subject[i] is in dataset self.idx_to_dataset[j].

    dok_matrix : 2D dok sparse matrix of shape [num_subj, num_dataset], 
        Dataset-subject membership matrix.
        It has the same content as self.matrix, but in Dictionary Of Keys format 
        for fast access of individual access.
    ...
    def add(self, file):
        ...
        is a list of strings with names of subjects in the dataset in the same 
        order as in the dataset. All datasets in all JSON files will be added 
        to the organizer. If some datasets are already in the organizer, the 
        information of those datasets will be replaced with this new version.
        ...
        if self.built_from_data is not None and self.built_from_data:
            raise Exception('This Dataset object was already initialized with fMRI datasets.')
        ...
        for m in mem:
            # sanity check
            if m['num_of_subj'] <= 0:
                raise Exception('Number of subjects in dataset ' + m['dataset'] + ' must be positive.')
            if m['num_of_subj'] != len(m['subjects']):
                raise Exception('Number of subjects in dataset ' + m['dataset'] + ' does not agree.')
            if m['dataset'] in new_ds or m['dataset'] in replace_ds:
                raise Exception('Dataset ' + m['dataset'] + ' appears more than once.')
            if len(m['subjects']) != len(set(m['subjects'])):
                raise Exception('Dataset ' + m['dataset'] + ' has duplicate subjects.')
            ...
            for subj in m['subjects']:
                if not subj in self.subject_to_idx:
                    new_sub.add(subj)
    ...
    def build_from_data(self, data):
        ...
        if not type(data) == dict:
            raise Exception('To build Dataset object from data, data must be a dict of dict ' 
                'where data[d][s] is the fMRI data of dataset d and subject s.')
        ...
        self.matrix = sp.coo_matrix((coo_data, (row, col)), shape=(self.num_subj, self.num_dataset))
    ...
    def remove_dataset(self, datasets):
        ...
        # sanity check
        for ds in datasets:
            if not ds in self.dataset_to_idx:
                raise Exception('Dataset '+ ds + ' does not exist.')

        # extract data from the sparse matrix
        data, row, col = self.matrix.data.tolist(), self.matrix.row.tolist(), self.matrix.col.tolist()
 
        # remove datasets from data
        data, row, col, subj_to_check = self._remove_datasets_from_data(datasets, data, row, col)
        ...
        self.matrix = sp.coo_matrix((data, (row, col)), shape=(self.num_subj, self.num_dataset))
        self.dok_matrix = self.matrix.todok(copy=True)
    ...
    def remove_subject(self, subjects):
        ...
        data, row, col = self._remove_subjects_from_data(subjects, data, row, col)
    ...
    def shared_subjects_between_datasets(self, ds1, ds2):
        ...
        subj1 = set(matrix_csc[:,self.dataset_to_idx[ds1]].indices) # indices of subjects in ds1
        subj2 = set(matrix_csc[:,self.dataset_to_idx[ds2]].indices) # indices of subjects in ds2
        return [self.idx_to_subject[subj] for subj in subj1.intersection(subj2)]
    ...
    def datasets_with_subject_all(self):
        ...
        ds_subj_list : dict of list of string, ds_subj_list[s] is a list where s 
        ...
        for subj in range(self.num_subj):
            ...
            ds_subj_list[subj_name] = [self.idx_to_dataset[ds] for ds in indices]
        return  ds_subj_list        
    ...
    def subjects_in_dataset(self, dataset):
        ...
        """
        #sanity check
        if not dataset in self.dataset_to_idx:
    ...
    def subjects_in_dataset_all(self):
        ...
        subj_ds_list : dict of list of string, subj_ds_list[d] is a list where d 
        ...
        for ds in range(self.num_dataset):
            ...
            subj_ds_list[ds_name] = [self.idx_to_subject[subj] for subj in indices]
        return  subj_ds_list
    ...
    def visualize_graph(self, font_size=14):
        ...
        # assign edge labels
        edge_labels=dict([((u,v,),self.adj_matrix[u,v]) for u,v in G.edges])
        pos=nx.spring_layout(G)
        nx.draw(G, pos=pos,with_labels=False)
        labels=nx.draw_networkx_labels(G,labels = self.idx_to_dataset, pos=pos, font_size=font_size)
        edge_labels=nx.draw_networkx_edge_labels(G,edge_labels=edge_labels, pos=pos, font_size=font_size)
        plt.show()                
        return
    ...
    def save(self, file):
        ...
        with open(file, 'wb') as f:
            pkl.dump(data, f, pkl.HIGHEST_PROTOCOL)
        print ('Dataset object saved to {}.'.format(file))
        return 
    ...
    def restore(self, file):
        ...
        for (k, v) in data.items():
            setattr(self, k, v)
        print ('Dataset object restored from {}.'.format(file))
        return
    ...
    def _compute_connected(self):
        ...
        None
        """
        # build the weighted adjacency matrix (how many shared subjects between each pair of datasets)
        matrix_csc = self.matrix.tocsc(copy=True)
        row, col, data = [], [], []
        for i in range(self.num_dataset):
            for j in range(i+1, self.num_dataset):
                ...
                if tmp != 0:
                    ...
                    col.extend([j,i])
                    data.extend([tmp, tmp])
        self.adj_matrix = sp.csc_matrix((data, (row, col)),shape=(self.num_dataset, self.num_dataset))
    ...
    def _construct_matrix(self, new_ds, new_sub, ds_dict):
        ...
        ds_dict : dict of list of string, ds_dict[d] is a list of subject names 
        in dataset d in the same order as in the dataset, where d is the name of 
        the dataset
    ...
    def _replace_dataset(self, replace_ds, ds_dict):
        """Replace information of some datasets with information in ds_dict assuming 
            those datasets are already in the organizer
        ...
        data, row, col, subj_to_check = self._remove_datasets_from_data(replace_ds, data, row, col)
    ...
    def _remove_subjects_from_data(self, subjects, data, row, col):
        ...
        Subjects are not re-indexed. Need to call _remove_subjects_by_re_indexing() 
        afterwards to re-index.
        """
        len_data = len(data)
        row_to_remove = set() # subject indices (row indices) to remove
        subjects = set(subjects)
        for subj in subjects:
            row_to_remove.add(self.subject_to_idx[subj])
        idx_to_remove = [] # data indices to remove from data, row, col lists
        ...
        # remove data
        data = [data[i] for i in range(len_data) if not i in idx_to_remove]
        row = [row[i] for i in range(len_data) if not i in idx_to_remove]
        col = [col[i] for i in range(len_data) if not i in idx_to_remove]          
    ...
    def _remove_datasets_from_data(self, datasets, data, row, col):
        ...
        Name of subjects that are possibly not in any datasets (and thus need 
        to be removed) after removal of those datasets

        Note
        ----

        Datasets are not re-indexed. Need to call _remove_datasets_by_re_indexing() 
        afterwards to re-index.
        """
        len_data = len(data)
        ...
        idx_to_remove = [] # data indices to remove from data, row, col lists
        subj_to_check = set() # possible subject indices to remove after removing datasets
    ...
    def _remove_subjects_by_re_indexing(self, subjects, row):
        ...
        for idx, r in enumerate(row):
            ...
            row[idx] = new_r
        # update mapping
        self.subject_to_idx, self.idx_to_subject = new_subject_to_idx, new_idx_to_subject
    ...
    def _remove_datasets_by_re_indexing(self, datasets, col):
        ...
        for idx, c in enumerate(col):
            ...
            col[idx] = new_c
        # update mapping
        self.dataset_to_idx, self.idx_to_dataset = new_dataset_to_idx, new_idx_to_dataset
