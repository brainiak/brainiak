...
from .utils.utils import phase_randomize, p_from_null, p_from_null_mem_saving
...
def isfc(D, collapse_subj=True, return_p=False,
         num_perm=1000, two_sided=False, random_state=0, float_type=np.float64): # add a parameter
    ...
    if return_p:
        n_perm = num_perm
		max_null = np.empty(num_perm).astype(float_type) # added
		min_null = np.empty(num_perm).astype(float_type) # added
    else:
        n_perm = 0
	
	
    #ISFC = np.zeros((n_vox, n_vox, n_subj, n_perm + 1)) # deleted
	ISFC = np.zeros((n_vox, n_vox, n_subj)).astype(float_type) # added
    ...
    for p in range(n_perm + 1):
        ...
        for loo_subj in range(D.shape[2]):
            group = np.mean(D[:, :, np.arange(n_subj) != loo_subj], axis=2)
            subj = D[:, :, loo_subj]
            #ISFC[:, :, loo_subj, p] = compute_correlation(group, subj) # deleted
			tmp_ISFC = compute_correlation(group, subj).astype(float_type) # added
			# Symmetrize matrix
			tmp_ISFC = (tmp_ISFC+tmp_ISFC.T)/2 # added
				
			#ISFC[:, :, loo_subj, p] = (ISFC[:, :, loo_subj, p] +
								   #ISFC[:, :, loo_subj, p].T) / 2 # deleted
			if p==0:
				ISFC[:, :, loo_subj] = tmp_ISFC # added
			else:
				leading_dims = tuple(np.arange(tmp_ISFC.ndim)) # added
				max_null[p-1]=np.max(tmp_ISFC, axis=leading_dims) # added
				min_null[p-1]=np.min(tmp_ISFC, axis=leading_dims) # added
    ...
    if return_p:
        # p = p_from_null(ISFC, two_sided) # deleted
        # return ISFC[..., 0], p # deleted
		p = p_from_null_mem_saving(ISFC, two_sided, max_null, min_null) # added
		return ISFC, p # added
    else:
        # return ISFC[..., 0] deleted
		return ISFC # added
