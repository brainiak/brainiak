...
def test_ISC():
    ...
    np.random.seed(0)
    (ISC, p) = brainiak.isfc.isc(D, return_p=True,
                                 num_perm=100, two_sided=True)
    assert np.isclose(ISC, [0.9540602, 0.0267954]).all(), \
...
def test_ISFC():
    ...
    np.random.seed(0)
    (ISFC, p) = brainiak.isfc.isfc(D, return_p=True,
                                   num_perm=100, two_sided=True)
