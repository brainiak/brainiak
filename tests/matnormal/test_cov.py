...
def test_Cov3FactorMaskedKron():
    ...
    with tf.Session() as sess:
        ...
        assert_allclose(logdet_np, cov.logdet.eval(session=sess), rtol=rtol, atol=atol)
        assert_allclose(sinv_np, cov.Sigma_inv.eval(session=sess)[np.ix_(mask_indices, mask_indices)], rtol=rtol, atol=atol)
        assert_allclose(sinvx_np, cov.Sigma_inv_x(X_tf).eval(session=sess)[mask_indices,:], rtol=rtol, atol=atol)
