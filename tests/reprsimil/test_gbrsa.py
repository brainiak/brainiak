...
def test_can_instantiate():
    ...
    s = brainiak.reprsimil.brsa.GBRSA(n_iter=40, n_iter_inner=4, rank=4,
...
def test_fit():
    ...
    noise_level = [None] * 3
    for i in range(3):
        noise_level[i] = np.random.rand(n_V[i])*(noise_top-noise_bot)+noise_bot
    ...
    for i in range(3):
        ...
        noise[i][0,:] = np.random.randn(n_V[i]) * noise_level[i] / np.sqrt(1-rho1[i]**2)
        for i_t in range(1,n_T[i]):
            noise[i][i_t,:] = noise[i][i_t-1,:] * rho1[i] +  np.random.randn(n_V[i]) * noise_level[i]
        noise[i] = noise[i] + np.dot(np.random.randn(n_T[i], 2), np.random.randn(2, n_V[i]))
    # baseline
    inten = [None] * 3
    for i in range(3):
    ...
    for i in range(3):
        ...
        betas_simulated[i] = np.dot(L_full,np.random.randn(n_C,n_V[i])) * sqrt_v
    ...
    for i in range(3):
        ...
        noise_new[i][0,:] = np.random.randn(n_V[i]) * noise_level[i] / np.sqrt(1-rho1[i]**2)
        for i_t in range(1,n_T[i]):
            noise_new[i][i_t,:] = noise_new[i][i_t-1,:] * rho1[i] +  np.random.randn(n_V[i]) * noise_level[i]

        Y_new[i] = signal[i] + noise_new[i] + inten[i]
    ts, ts0 = gbrsa.transform(Y_new,scan_onsets=scan_onsets)
    [score, score_null] = gbrsa.score(X=Y_new, design=design_mat, scan_onsets=scan_onsets)
    [score_noise, score_null_noise] = gbrsa.score(X=noise_new, design=design_mat, scan_onsets=scan_onsets)
    for i in range(3):
        assert np.shape(ts[i]) == (n_T[i], n_C) and np.shape(ts0[i]) == (n_T[i], n_nureg+1)
        ...
        assert score[i] > score_null[i], "Full model does not win over null model on data containing signal"

        
        assert score_noise[i] < score_null_noise[i], "Null model does not win over full model on data without signal"

    [score, score_null] = gbrsa.score(X=[None]*3, design=design_mat, scan_onsets=scan_onsets)
    assert score == [None] * 3 and  score_null == [None] * 3, "score did not return list of None when data is None"
...
def test_gradient():
    ...
    SNR_bins = 11
    rho_bins = 20
    gbrsa = GBRSA(verbose=False,n_iter = 3,rank=n_C, SNR_bins=SNR_bins,
    ...
    for i in range(3):
        ...
        s2XTAcorrX[i] = np.dot(design_mat[i].T, design_mat[i]) * s[:, None, None]**2  * a[:, None, None]
        sXTAcorrY[i] = np.dot(design_mat[i].T, Y[i])  * s[:, None, None]  * a[:, None, None]
