...
def test_against_scipy_mvn_row_marginal():
    ...
    with tf.Session() as sess: 
        ...
        tf_answer = modelbase.matnorm_logp_marginal_row(X_tf, rowcov, colcov, A_tf, Q).eval(session=sess)
...
def test_against_scipy_mvn_col_marginal():
    ...
    with tf.Session() as sess: 
        ...
        scipy_answer = np.sum(multivariate_normal.logpdf(X, np.zeros([m]), colcov_np))

        tf_answer = modelbase.matnorm_logp_marginal_col(X_tf, rowcov, colcov, A_tf, Q).eval(session=sess)
        assert_allclose(scipy_answer, tf_answer, rtol=rtol)
