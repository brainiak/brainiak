...
def test_brsa_rudimentary():
    ...
    U = gen_U_nips2016_example() # this is Mingbo's synth example from the paper
    ...
    tr, sz = gen_brsa_data_from_model(U, n_T = n_T , n_V = n_V, n_nureg = 2, space_cov=spacecov_true, time_cov=timecov_true)

    spacecov_model = NoiseCovIdentity(size=n_V)
    timecov_model = NoiseCovDiagonal(size=n_T)
    model_matnorm = MatnormBRSA(n_TRs = n_T, n_V = n_V, n_C=16, n_nureg=2, time_noise_cov=timecov_model, space_noise_cov=spacecov_model, learnRate=0.1)
