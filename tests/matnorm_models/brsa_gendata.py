...
def gen_brsa_data_brainiak_example(design, n_run):
    ...
    # The standard deviation of the noise is in the range of
    # [noise_bot, noise_top]
    # In fact, we simulate autocorrelated noise with AR(1) model.
    # So the noise_level reflects the independent additive noise at
    # each time point (the "fresh" noise)
    ...
    train = {"beta": betas_simulated, "X": design.design_task, "Y": Y,
             "U": ideal_cov, "coords": coords_flat, "scan_onsets": scan_onsets,
             "inten": inten}
